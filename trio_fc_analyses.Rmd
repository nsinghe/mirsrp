...
df_cog = read_excel(file.path(df_path, "AD_DF_2022/module_freeze_nov21/clinical_core_nov21/psychometrics.xlsx"))%>%
  mutate(MAPID = as.factor(ID))%>%
  mutate(date_cog = psy_date)

df_cog = subset(df_cog, select = -ID)
...
tau_data_merge_trio <- merge(tau_data_merge_trio, df_cog, by="MAPID", all.x = TRUE) %>%
  mutate(abs_diff_cog=abs(as.Date(date_cog)-as.Date(date_tau))) %>%
  #filter(abs_diff < (365.25*3)) %>% 
  arrange(abs_diff_cog) %>% 
  dplyr::group_by(MAPID, date_tau) %>% 
  dplyr::slice(1)
...
#Dimensions should be 729 obs of 3500 variables (as of 8/11)
...
#extended analyses:
data_trio_cdr = tau_data_merge_trio%>%
  filter(abs_diff_cdr < (365.25*4))
...
#extended analyses:
data_trio_cdr = data_trio_cdr%>%
  dplyr::mutate(count_final = "1")%>%
  dplyr::group_by(MAPID)%>%
  dplyr::arrange(date_fc)%>%
  dplyr::mutate(visit_final = cumsum(count_final))
...
data_trio_cdr01 <- rbind(data_trio, data_trio_cdr)
#FINAL SAMPLE = 149 + 16 = 165
...
data_trio_cdr01 <- data_trio_cdr01%>%
  mutate(age_tau=abs(as.Date(BIRTH,format = "%m/%d/%Y")-as.Date(date_tau,format = "%m/%d/%Y"))/365.25)
data_trio_cdr01$age_tau <- as.numeric(data_trio_cdr01$age_tau)
...
#CODE FOR HEATMAP MATRICES
##create functions to speed up analysis
```{r}
#create new df
maxNeg = min(data_trio$Centiloid_merge)
df_heatmap_trio <- data_trio %>%
  mutate(age_tau=abs(as.Date(BIRTH,format = "%m/%d/%Y")-as.Date(date_tau,format = "%m/%d/%Y"))/365.25) %>%
  mutate(logCentiloidplus = log(Centiloid_merge - maxNeg + 3)) %>%
  mutate(logTau = log(Tauopathy)) -> df_heatmap_trio

df_heatmap_trio$centCategory <- cut(df_heatmap_trio$Centiloid_merge, breaks = c(-Inf, 20.6, +Inf), labels = c("Cent-", "Cent+"))
table(df_heatmap_trio$centCategory) #sanity check

df_heatmap_trio$at_int = df_heatmap_trio$Centiloid_merge * df_heatmap_trio$Tauopathy
df_heatmap_trio$logat_int = df_heatmap_trio$logCentiloidplus * df_heatmap_trio$logTau

hm_final_trio = df_heatmap_trio %>% dplyr::select(MAPID,
...
```{r}
###create more functions
all(is.na(hm_final_trio$logCentiloidplus))
...
#extended analyses after final presentation
```{r}
#create a new grouping

data_trio_cdr01$PibCategory <- cut(data_trio_cdr01$pib_fsuvr_rsf_tot_cortmean, breaks = c(-Inf, 1.42, +Inf), labels = c("Aneg", "Apos"))
data_trio_cdr01$av45Category <- cut(data_trio_cdr01$av45_fsuvr_rsf_tot_cortmean, breaks = c(-Inf, 1.19, +Inf), labels = c("Aneg", "Apos"))

data_trio_cdr01$AmyloidCategory <-with(data_trio_cdr01, ifelse(
                                    cent_tracer == 'PiB' & PibCategory == 'Apos', 'Apos', ifelse(
                                    cent_tracer == 'PiB' & PibCategory == 'Aneg', 'Aneg', ifelse(
                                    cent_tracer == 'AV45' & av45Category == 'Apos', 'Apos', ifelse(
                                    cent_tracer == 'AV45' & av45Category == 'Aneg', 'Aneg', 'NA')))))


data_trio_cdr01$TauCategory <- cut(data_trio_cdr01$Tauopathy, breaks = c(-Inf, 1.22, +Inf), labels = c("TauNegative", "TauPositive")) 
table(data_trio_cdr01$TauCategory) #124 TauNeg & 86 TauPos

data_trio_cdr01$AmyloidTauCategory <-with(data_trio_cdr01, ifelse(
                                    AmyloidCategory == 'Aneg' & Tauopathy < 1.22 & CDR == 0, 'A-T-', ifelse(
                                    AmyloidCategory == 'Apos' & Tauopathy < 1.22 & CDR == 0, 'A+T-', ifelse(
                                    AmyloidCategory == 'Aneg' & Tauopathy > 1.22 & CDR == 0, 'A-T+', ifelse(
                                    AmyloidCategory == 'Apos' & Tauopathy > 1.22 & CDR == 0, 'A+T+', 'Symptomatic AD')
                      ))))
table(data_trio_cdr01$AmyloidTauCategory) # 80 A-T-, 31 A-T+, 13 A+T-, 25 A+T+, 16 Symptomatic AD
...
```{r}
#SAL x SAL analysis
kruskal.test(data_trio_cdr01$SAL_x_SAL ~ data_trio_cdr01$AmyloidTauCategory,  data = data_trio_cdr01)
compare_means(SAL_x_SAL ~ AmyloidTauCategory,  data = data_trio_cdr01, method = "wilcox")
...
#Violin Plot of SAL x SAL
p<-ggplot(data_trio_cdr01, aes(x=AmyloidTauCategory, y=SAL_x_SAL, fill=AmyloidTauCategory)) +
  labs(title="TRIO DATASET SAL FC",x="", y = "SAL x SAL FC") +
  geom_violin(trim=FALSE, color = "black") + 
  scale_fill_manual(breaks=c("A-T-", "A+T-", "A-T+", "A+T+", "Symptomatic AD"),
                    values=c("blue", "yellow", "forestgreen", "orange", "red")) +
...
p
ggsave('SAL_violin_plot.png', width = 5, height = 5, units = "in", dpi = 320)
...
```{r}
#DAN x DAN analysis
kruskal.test(data_trio_cdr01$DAN_x_DAN ~ data_trio_cdr01$AmyloidTauCategory,  data = data_trio_cdr01)
compare_means(DAN_x_DAN ~ AmyloidTauCategory,  data = data_trio_cdr01, method = "wilcox")
my_comparisons <- list( #c("A-T-", "A+T-"), c("A+T-", "A+T+"), c("A-T-", "A+T+"),
   c("A-T-", "A+T-")
  #, c("A+T-", "A+T+"), 
  #c("A-T-", "A+T+")
  ...
  #c("A-T-", "A-T+"), c("A-T+", "A+T-")
  #c("A+T-", "A+T+"), 
  ,c("A-T-", "Symptomatic AD"), c("A-T+", "Symptomatic AD"), c("Symptomatic AD", "A+T-"), c("Symptomatic AD", "A+T+")
  
)

#Violin Plot of DAN x DAN
p<-ggplot(data_trio_cdr01, aes(x=AmyloidTauCategory, y=DAN_x_DAN, fill=AmyloidTauCategory)) +
  labs(title="TRIO DATASET: DAN FC",x="", y = "DAN x DAN FC") +
  geom_violin(trim=FALSE, color = "black") + 
  scale_fill_manual(breaks=c("A-T-", "A+T-", "A-T+", "A+T+", "Symptomatic AD"),
                    values=c("blue", "yellow", "forestgreen", "orange", "red")) +
  ...
  theme(legend.position="none", plot.title = element_text(hjust = 0.5))
p
ggsave('DAN_violin_plot.png', width = 5, height = 5, units = "in", dpi = 320)
```

##DMN
```{r}
#DMN x DMN analysis
kruskal.test(data_trio_cdr01$DMN_x_DMN ~ data_trio_cdr01$AmyloidTauCategory,  data = data_trio_cdr01)
compare_means(DMN_x_DMN ~ AmyloidTauCategory,  data = data_trio_cdr01, method = "wilcox")
my_comparisons <- list( #c("A-T-", "A+T-"), c("A+T-", "A+T+"), c("A-T-", "A+T+"),
   c("A-T-", "A+T-")
  #, c("A+T-", "A+T+"), 
  #c("A-T-", "A+T+")
...
#Violin Plot of DMN x DMN
p<-ggplot(data_trio_cdr01, aes(x=AmyloidTauCategory, y=DMN_x_DMN, fill=AmyloidTauCategory)) +
  labs(title="TRIO DATASET: DMN FC",x="", y = "DMN x DMN FC") +
  geom_violin(trim=FALSE, color = "black") + 
  scale_fill_manual(breaks=c("A-T-", "A+T-", "A-T+", "A+T+", "Symptomatic AD"),
                    values=c("blue", "yellow", "forestgreen", "orange", "red")) +
  ...
  theme(legend.position="none", plot.title = element_text(hjust = 0.5))
p
ggsave('DMN_violin_plot.png', width = 5, height = 5, units = "in", dpi = 320)
```

## CO
```{r}
kruskal.test(data_trio_cdr01$CO_x_CO ~ data_trio_cdr01$AmyloidTauCategory,  data = data_trio_cdr01)
compare_means(CO_x_CO ~ AmyloidTauCategory,  data = data_trio_cdr01, method = "wilcox")
my_comparisons <- list( #c("A-T-", "A+T-"), c("A+T-", "A+T+"), c("A-T-", "A+T+"),
  # c("A-T-", "A+T-")
  #, c("A+T-", "A+T+"), 
  #c("A-T-", "A+T+")
  #, c("A+T+", "CDR > 0"), c("A+T-", "CDR > 0"), c("A-T-", "CDR > 0")
  c("A-T-", "A+T-"), c("A+T-", "Symptomatic AD")
)

#Violin Plot of CO x CO
p<-ggplot(data_trio_cdr01, aes(x=AmyloidTauCategory, y=CO_x_CO, fill=AmyloidTauCategory)) +
  labs(title="TRIO DATASET: CO FC",x="", y = "CO x CO FC") +
  geom_violin(trim=FALSE, color = "black") + 
  scale_fill_manual(breaks=c("A-T-", "A+T-", "A-T+", "A+T+", "Symptomatic AD"),
                    values=c("blue", "yellow", "forestgreen", "orange", "red")) +
  ...
  theme(legend.position="none", plot.title = element_text(hjust = 0.5))
p
ggsave('CO_violin_plot.png', width = 5, height = 5, units = "in", dpi = 320)
```

## FP
```{r}
kruskal.test(data_trio_cdr01$FP_x_FP ~ data_trio_cdr01$AmyloidTauCategory,  data = data_trio_cdr01)
compare_means(FP_x_FP ~ AmyloidTauCategory,  data = data_trio_cdr01, method = "wilcox")
my_comparisons <- list( #c("A-T-", "A+T-"), c("A+T-", "A+T+"), c("A-T-", "A+T+"),
  # c("A-T-", "A+T-")
  #, c("A+T-", "A+T+"), 
  #c("A-T-", "A+T+")
...
#Violin Plot of CO x CO
p<-ggplot(data_trio_cdr01, aes(x=AmyloidTauCategory, y=FP_x_FP, fill=AmyloidTauCategory)) +
  labs(title="TRIO DATASET: FP FC",x="", y = "FP x FP FC") +
  geom_violin(trim=FALSE, color = "black") + 
  scale_fill_manual(breaks=c("A-T-", "A+T-", "A-T+", "A+T+", "Symptomatic AD"),
                    values=c("blue", "yellow", "forestgreen", "orange", "red")) +
  ...
  theme(legend.position="none", plot.title = element_text(hjust = 0.5))
p
ggsave('FP_violin_plot.png', width = 5, height = 5, units = "in", dpi = 320)
```
...
#cognitive analyses
```{r}
#Basic code to form cognitive composite from individual measures
#Global Composite
#srtfree, Trails A, Trails B, ANIMALS Naming
data_trio_cdr01$z_global_srt = scale(data_trio_cdr01$srtfree)
data_trio_cdr01$z_global_animal = scale(data_trio_cdr01$ANIMALS)
data_trio_cdr01$z_global_tma = -1*(scale(data_trio_cdr01$tma))
data_trio_cdr01$z_global_tmb = -1*(scale(data_trio_cdr01$tmb))

data_trio_cdr01$global_comp <- rowMeans(subset(data_trio_cdr01, select = c(
  z_global_srt,
  z_global_animal,
  z_global_tma,
  z_global_tmb
...
##DMN
```{r}
#Regression model to test interactive relationship between cognition and FC measure across different clinical groups (e.g., A-T-, A+T-, etc)
cog_error_fc_cdr = lm((global_comp) ~ #lumi_ab42_dr17
                     #agegap_fc2*group_multi
                     #agegap_fc2*control
                      AmyloidTauCategory*DMN_x_DMN
                     # agegap_fc2*impaired
                   ...
                   , data = data_trio_cdr01)
summary(cog_error_fc_cdr)
#Calculate effect sizes of regression model terms

#etasq(cog_error_fc_cdr)

#Plot relationships between cognition and FC measure
##Subgroup-specific relationships in colors
##Overall cross-sectional main effect in black

g = ggplot(data_trio_cdr01, aes(y = global_comp, x = DMN_x_DMN))
g + geom_point(aes(color = AmyloidTauCategory)) +
  labs(title="", x = "TRIO: Residual DMN x DMN", y = "Global Cognition (composite)") +
  geom_smooth(aes(color = AmyloidTauCategory, fill = AmyloidTauCategory), method = glm) +
  geom_smooth(color = 'black', method = glm, linetype = 'dashed') +
  scale_color_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  scale_fill_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  ...
  theme(legend.position=c(.85, 0.21), plot.title = element_text(hjust = 0.5), legend.title = element_blank(), legend.key = element_blank(), legend.background=element_blank())
```
...
g = ggplot(data_trio_cdr01, aes(y = global_comp, x = SAL_x_SAL))
g + geom_point(aes(color = AmyloidTauCategory)) +
  labs(title="", x = "TRIO: Residual SAL x SAL", y = "Global Cognition (composite)") +
  geom_smooth(aes(color = AmyloidTauCategory, fill = AmyloidTauCategory), method = glm) +
  geom_smooth(color = 'black', method = glm, linetype = 'dashed') +
  scale_color_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  scale_fill_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
...
g = ggplot(data_trio_cdr01, aes(y = global_comp, x = DAN_x_DAN))
g + geom_point(aes(color = AmyloidTauCategory)) +
  labs(title="", x = "TRIO: Residual DAN x DAN", y = "Global Cognition (composite)") +
  geom_smooth(aes(color = AmyloidTauCategory, fill = AmyloidTauCategory), method = glm) +
  geom_smooth(color = 'black', method = glm, linetype = 'dashed') +
  scale_color_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  scale_fill_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
...
g = ggplot(data_trio_cdr01, aes(y = global_comp, x = CO_x_CO))
g + geom_point(aes(color = AmyloidTauCategory)) +
  labs(title="", x = "TRIO: Residual CO x CO", y = "Global Cognition (composite)") +
  geom_smooth(aes(color = AmyloidTauCategory, fill = AmyloidTauCategory), method = glm) +
  geom_smooth(color = 'black', method = glm, linetype = 'dashed') +
  scale_color_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  scale_fill_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
...
g = ggplot(data_trio_cdr01, aes(y = global_comp, x = FP_x_FP))
g + geom_point(aes(color = AmyloidTauCategory)) +
  labs(title="", x = "TRIO: Residual FP_x_FP", y = "Global Cognition (composite)") +
  geom_smooth(aes(color = AmyloidTauCategory, fill = AmyloidTauCategory), method = glm) +
  geom_smooth(color = 'black', method = glm, linetype = 'dashed') +
  scale_color_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  scale_fill_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
...
#CSF analyses
```{r}
hist(data_trio_cdr01$CSF_sTREM2, breaks=20)
```

##DMN
```{r}
csf_error_fc_cdr = lm((CSF_sTREM2) ~ #lumi_ab42_dr17
                     #agegap_fc2*group_multi
                     #agegap_fc2*control
                      AmyloidTauCategory*DMN_x_DMN
                     # agegap_fc2*impaired
...
g = ggplot(data_trio_cdr01, aes(y = CSF_sTREM2, x = DMN_x_DMN))
g + geom_point(aes(color = AmyloidTauCategory)) +
  labs(title="", x = "TRIO Residual DMN x DMN", y = "CSF sTREM2") +
  geom_smooth(aes(color = AmyloidTauCategory, fill = AmyloidTauCategory), method = glm) +
  geom_smooth(color = 'black', method = glm, linetype = 'dashed') +
  scale_color_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  scale_fill_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  ...
  #ylim(min(c(data$Age, yscale)), max(c(data$Age, yscale))) +
  theme_bw(base_size = 15) +
  theme(legend.position=c(.15, .95), plot.title = element_text(hjust = 0.5), legend.title = element_blank(), legend.key = element_blank(), legend.background=element_blank())
```
...
g = ggplot(data_trio_cdr01, aes(y = CSF_sTREM2, x = SAL_x_SAL))
g + geom_point(aes(color = AmyloidTauCategory)) +
  labs(title="", x = "TRIO: Residual SAL x SAL", y = "CSF sTREM2") +
  geom_smooth(aes(color = AmyloidTauCategory, fill = AmyloidTauCategory), method = glm) +
  geom_smooth(color = 'black', method = glm, linetype = 'dashed') +
  scale_color_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  scale_fill_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
...
g = ggplot(data_trio_cdr01, aes(y = CSF_sTREM2, x = DAN_x_DAN))
g + geom_point(aes(color = AmyloidTauCategory)) +
  labs(title="", x = "TRIO: Residual DAN x DAN", y = "CSF sTREM2") +
  geom_smooth(aes(color = AmyloidTauCategory, fill = AmyloidTauCategory), method = glm) +
  geom_smooth(color = 'black', method = glm, linetype = 'dashed') +
  scale_color_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  scale_fill_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
...
g = ggplot(data_trio_cdr01, aes(y = CSF_sTREM2, x = CO_x_CO))
g + geom_point(aes(color = AmyloidTauCategory)) +
  labs(title="", x = "TRIO: Residual CO x CO", y = "CSF sTREM2") +
  geom_smooth(aes(color = AmyloidTauCategory, fill = AmyloidTauCategory), method = glm) +
  geom_smooth(color = 'black', method = glm, linetype = 'dashed') +
  scale_color_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  scale_fill_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
...
g = ggplot(data_trio_cdr01, aes(y = CSF_sTREM2, x = FP_x_FP))
g + geom_point(aes(color = AmyloidTauCategory)) +
  labs(title="", x = "TRIO: Residual FP x FP", y = "CSF sTREM2") +
  geom_smooth(aes(color = AmyloidTauCategory, fill = AmyloidTauCategory), method = glm) +
  geom_smooth(color = 'black', method = glm, linetype = 'dashed') +
  scale_color_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
  scale_fill_manual(values = c("blue", "forestgreen", "yellow", "orange", "red")) +
